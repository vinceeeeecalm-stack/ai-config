...
| Public Netlify UI and API | Production Netlify deploy `6a3770f49529294af075ee70` is ready. Static `relay-chat.js` serves version `2026.06.21.8`, and `/api/relay/status` returns `ok: true`. A later deploy attempt `6a37729c8ad80d76f3966a44` was skipped by Netlify because account credit usage was exceeded, so the final expanded local cleanup-name matcher is installed locally but not published to Netlify yet. The old `mobile-relay` function name is now compatibility-wrapped to the current implementation. | Complete with deploy-credit note |
...
local queue: queued_commands=0, unresolved_commands=0, active_devices=19
public queue: queued_commands=0, unresolved_commands=0, active_devices=8
tunnel URL: https://da82eae64ea198.lhr.life/relay-chat.html
...
- Current public tunnel UI: `https://da82eae64ea198.lhr.life/relay-chat.html`
