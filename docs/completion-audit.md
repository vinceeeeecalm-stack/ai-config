...
| --- | --- | --- |
| Minimal phone UI | Local, LAN, and tunnel pages serve the WeChat-like `public/relay-chat.html` shell at version `2026.06.21.11`; the current Netlify deploy still serves version `2026.06.21.8` until account credits allow redeploy. The phone page includes `状态`, `唤醒`, `清除旧登录`, top-bar queue/sync indicators, transcript recovery, duplicate-click protection, reset support via `?reset=1`, cloud token revocation on reset, and a compact route-advice banner from `/api/relay/app-info`. The desktop pairing page shows local/stable/tunnel app versions before pairing. | Complete |
