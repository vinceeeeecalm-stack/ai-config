...
| --- | --- | --- |
| Minimal phone UI | Local, LAN, tunnel, and Netlify pages serve the WeChat-like `public/relay-chat.html` shell. The deployed phone page includes version `2026.06.21.2`, `状态`, `唤醒`, `清除旧登录`, top-bar queue/sync indicators, transcript recovery, and reset support via `?reset=1`. | Complete |
| Daily repeated use | Server API requests are serialized in `scripts/cloud-relay-standalone-server.mjs`; the phone UI sends through a client-side queue in `public/relay-chat.js`; `GET /api/relay/mobile/transcript` restores recent bidirectional history after refresh; tests include concurrent 6-message preservation and transcript device scoping. | Complete |
...
tunnel URL: https://f6cfb1571cec63.lhr.life/relay-chat.html
...
- Current public tunnel UI: `https://f6cfb1571cec63.lhr.life/relay-chat.html`
