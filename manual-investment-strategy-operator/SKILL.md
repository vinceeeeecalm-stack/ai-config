...
当前生产规则仍处于阶段 0；阶段 1 衍生品前移只在影子层持续采样和七天结算：

- `scripts/unified_short_term_opportunity.py` 提供七个确定性合同、三组冻结阈值的无前视基线回放、持续候选追加式记忆、排名/持有时钟影子计算和统一真钱资金池归因。
- `scripts/unified_shortterm_phase_readiness.py` 是全目标阶段审计入口。它只读取冻结发布、独立复验、阶段一运行账本和自动化状态，输出每层的真实就绪等级、全部阻断和唯一下一步；不得用文件存在或局部 PASS 推断整个赚钱闭环完成。
