...
本 policy 用于长期新增资金和 DCA 决策。核心目标不是维持固定比例，而是在不破坏生存能力的前提下，提高以下目标的达成概率：

- 5年组合达到 10x。
- 10年组合达到 10x 以上。
- 用少数高质量高凸性资产提高上行空间，但不让未验证资产拖垮长期核心仓。

5年10x 约等于年化 58%。10年10x 约等于年化 26%。因此，新增资金必须优先考虑资本效率、长期增长弹性、质押复利、流动性、供应压力和数据质量。报告必须先量化当前组合距离目标的差距，再推荐资产。
...
- 每次推荐必须先比较当前已有 `ETH/lcETH、SOL、ADA` 及其他实质持仓，再决定是否需要新资产。新增资产默认采用“替代一个较弱目的地”，而不是在原有资产之外继续叠加。
...
- ETH/lcETH 是否超过组合目标区间，若超过则不新增 ETH。
- SOL 是否仍低配，且质押收益、生态活跃度、资金流、链上数据和价格位置是否支持新增。
- NIGHT 是否具有长期高凸性 thesis 支撑，且流动性、解锁压力和官方路线图没有恶化。
- ADA 是否只是因为 APY 被过度加仓。
- BTC 是否真的触发极端低估、恐慌或流动性锚条件。
...
ETH/lcETH 的特殊规则：当 ETH 是高质量核心资产但当前组合已明显超配时，报告必须同时说明“长期可持有”和“新增 DCA 效率偏低”。这类资产可标记为 `quality_hold`，但新增资金应优先让位给更能提高 5年/10年 10x 概率的低配增长仓。

## SOL 规则

SOL 是长期增长主线候选，但不机械满配。

必须同时评估：

- 质押收益和复投价值。
- 生态活跃度、资金流、链上数据和开发者/应用增长。
- funding、OI、成交量、价格位置和回撤程度。
- 链上安全、宕机、监管或流动性风险。

当 SOL 低配且市场回调、funding 冷却、数据 verified 时，提高 DCA 比例；当 SOL 过热或短线已大幅反弹时，降低立即买入比例，转为挂单或机会仓。

## 其他资产规则

- ADA 因约 3% APY、深度折价和质押复利属性作为长期卫星仓；当价格处于历史深度回撤、数据 verified、质押可持续且当前仓位未超配时，可上调到 `15%-22% crypto target`。若生态/TVL/开发者/资金流持续弱于 SOL，则保持卫星定位，不升级为唯一主仓。
- BTC 默认不是新增 DCA 主线，仅在极端低估、系统性恐慌、组合需要流动性锚或增长仓被风险模型 block 时启用。
- ETH/lcETH 是已有核心仓；当超过目标区间时不新增，只跟踪 Coinbase staking、lcETH 估值、解锁、费用、折价/溢价和可流动性。
- NEAR/SUI/LINK/TRX 等候选必须根据长期 thesis、数据质量、流动性和持仓适配动态进入或退出，不因固定清单自动推荐。
