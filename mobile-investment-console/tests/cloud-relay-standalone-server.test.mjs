...
test("standalone cloud relay serves the PWA and relay API", async () => {
  ...
  const localtunnelStatusPath = path.join(tempDir, "localtunnel-status.json");
  await fs.writeFile(localtunnelStatusPath, JSON.stringify({
    ok: true,
    public_url: "https://codex-relay-vince.loca.lt/relay-chat.html",
    health: { ok: true }
  }));
  ...
  const server = createStandaloneRelayServer({
    ...
    env: {
      ...
      LOCALTUNNEL_STATUS_PATH: localtunnelStatusPath,
  ...
  try {
    ...
    assert.equal(pairing.temporary_tunnel_source, "localhostrun");
    assert.equal(pairing.public_urls.some((item) => item.source === "localhostrun" && item.healthy), true);
    assert.equal(pairing.public_urls.some((item) => item.source === "localtunnel" && item.healthy), true);
    assert.equal(pairing.app_versions.local.version, "2026.06.21.12");
    assert.equal(pairing.app_versions.stable_public.error, "disabled");
    assert.equal(pairing.app_versions.temporary_tunnel.version, "2026.06.21.12");
    ...
    assert.equal(appInfo.app_versions.local.version, "2026.06.21.12");
    assert.equal(appInfo.app_versions.temporary_tunnel.version, "2026.06.21.12");
    ...
    assert.equal(appInfo.temporary_tunnel_url, "https://temporary-test.lhr.life/relay-chat.html");
    assert.equal(appInfo.temporary_tunnel_source, "localhostrun");
