...
const localtunnelStatusPath = path.join(installDir, "state", "localtunnel-status.json");
...
async function checkPublicTunnel(checks) {
  await record(checks, "public_tunnel_status", false, async () => {
    const appInfo = await fetchJson(`${localBaseUrl}/api/relay/app-info`);
    const localtunnelStatus = readJson(localtunnelStatusPath, null);
    const localhostrunStatus = readJson(localhostrunStatusPath, null);
    const tunnelUrl = appInfo.temporary_tunnel_url || null;
    if (!tunnelUrl) return { ok: false, warning: true, evidence: { reason: "temporary tunnel status missing from app-info" } };
    const source = appInfo.temporary_tunnel_source || inferTunnelSource(tunnelUrl);
    const status = source === "localtunnel" ? localtunnelStatus : source === "localhostrun" ? localhostrunStatus : null;
    const health = await fetchJson(relayStatusUrlForAppUrl(tunnelUrl));
    const statusFileOk = status ? Boolean(status.ok) : null;
    const ok = Boolean(health.ok && statusFileOk !== false);
    return {
      ok,
      warning: !ok,
      evidence: {
        ...
        source,
        status_file_ok: statusFileOk,
        localtunnel_launchd_loaded: Boolean(localtunnelStatus?.launchd_loaded),
        localhostrun_launchd_loaded: Boolean(localhostrunStatus?.launchd_loaded),
        localhostrun_health_launchd_loaded: Boolean(localhostrunStatus?.health_launchd_loaded),
        temporary_tunnel_app_version: appInfo.app_versions?.temporary_tunnel?.version || null,
        recommended_source: appInfo.app_versions?.recommended_source || null,
...
function relayStatusUrlForAppUrl(url) {
  const parsed = new URL(url);
  parsed.pathname = "/api/relay/status";
  parsed.search = "";
  parsed.hash = "";
  return String(parsed);
}

function inferTunnelSource(url) {
  const host = new URL(url).hostname;
  if (host.endsWith(".loca.lt")) return "localtunnel";
  if (host.endsWith(".lhr.life")) return "localhostrun";
  return "unknown";
}
