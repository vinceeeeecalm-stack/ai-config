...
export function createStandaloneRelayServer(options = {}) {
  ...
  const env = {
    ...
    LOCALTUNNEL_STATUS_PATH: options.env?.LOCALTUNNEL_STATUS_PATH
      ?? process.env.LOCALTUNNEL_STATUS_PATH
      ?? path.join(path.dirname(statePath), "localtunnel-status.json")
  };
...
async function localPairingStatus(req, store, env, publicDir) {
  ...
  return {
    ...
    body: {
      ...
      temporary_tunnel_source: publicRelay.temporary_source,
...
async function publicRelayInfo(env) {
  ...
  const localtunnelStatus = await readJsonIfExists(env.LOCALTUNNEL_STATUS_PATH);
  const localhostrunStatus = await readJsonIfExists(env.LOCALHOSTRUN_STATUS_PATH);
  const tunnelCandidates = [
    tunnelCandidate("localhostrun", "localhost.run", localhostrunStatus, 10 * 60 * 1000),
    tunnelCandidate("localtunnel", "localtunnel", localtunnelStatus, 90 * 1000)
  ].filter((item) => item.url);
  const bestTunnel = tunnelCandidates.find((item) => item.direct_local) || tunnelCandidates[0] || null;
  const urls = [];
  if (stableUrl) urls.push({ kind: "stable", label: "Netlify", url: stableUrl });
  tunnelCandidates
    .filter((item) => item.url !== stableUrl)
    .forEach((item) => urls.push({ kind: "temporary", label: item.label, source: item.source, url: item.url, healthy: item.direct_local, fresh: item.fresh }));
  return {
    primary_url: stableUrl || bestTunnel?.url || null,
    primary_source: stableUrl ? "stable" : bestTunnel ? "temporary" : null,
    stable_url: stableUrl || null,
    temporary_url: bestTunnel?.url || null,
    temporary_source: bestTunnel?.source || null,
    temporary_direct_local: Boolean(bestTunnel?.direct_local),
    urls
  };
}

function tunnelCandidate(source, label, status, maxAgeMs) {
  const url = relayAppUrl(status?.public_url);
  const fresh = statusFresh(status, maxAgeMs);
  return {
    source,
    label,
    url,
    fresh,
    direct_local: Boolean(url && status?.ok && status?.health?.ok && fresh)
  };
}

function statusFresh(status, maxAgeMs) {
