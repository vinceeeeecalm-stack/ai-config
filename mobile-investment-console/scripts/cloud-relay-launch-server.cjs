...
process.env.LOCALTUNNEL_STATUS_PATH = process.env.LOCALTUNNEL_STATUS_PATH || path.join(installDir, "state", "localtunnel-status.json");
process.env.LOCALHOSTRUN_STATUS_PATH = process.env.LOCALHOSTRUN_STATUS_PATH || path.join(installDir, "state", "localhostrun-status.json");
...
import("./cloud-relay-standalone-server.mjs").then(({ createStandaloneRelayServer }) => {
  ...
  const server = createStandaloneRelayServer({
    ...
    env: {
      ...
      LOCALTUNNEL_STATUS_PATH: process.env.LOCALTUNNEL_STATUS_PATH,
      LOCALHOSTRUN_STATUS_PATH: process.env.LOCALHOSTRUN_STATUS_PATH
    }
  });
