...
The stable daily public path is Netlify. When Netlify is stale or unavailable, the installed `localhost.run` tunnel is the preferred direct fallback to the local relay because it has a LaunchAgent health check and auto-repair loop:
...
```text
https://004d37c6e2e5a6.lhr.life/relay-chat.html
```
...
The `localtunnel` URL remains a secondary fallback. Its requested subdomain is best-effort and may become stale, so app-info only recommends it when its status file is fresh and healthy:

```sh
cd "/Users/vincentpan/Library/Application Support/CodexRelayCloud"
node scripts/cloud-relay-localtunnel-install.cjs status
node scripts/cloud-relay-localtunnel-install.cjs install
...
The reset flow revokes the current cloud mobile token, then removes the saved local token, cursor, pending reply cache, unsent outbox, old service worker, and old PWA caches for this origin. After reset, the local/LAN/tunnel setup page should show version `2026.06.21.12`; the current stable Netlify deploy can still show version `2026.06.21.8` until account credits allow the next deploy. Make sure the mode matches the code you enter before pairing.
