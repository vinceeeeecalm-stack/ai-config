...
| --- | --- | --- |
| Minimal phone UI | Local, LAN, and tunnel pages serve the WeChat-like `public/relay-chat.html` shell at version `2026.06.21.12`; the current Netlify deploy still serves version `2026.06.21.8` until account credits allow redeploy. The phone page includes `状态`, `唤醒`, `清除旧登录`, top-bar queue/sync indicators, transcript recovery, duplicate-click protection, reset support via `?reset=1`, cloud token revocation on reset, and a compact route-advice banner from `/api/relay/app-info`. App-info marks a healthy direct `localhost.run` tunnel as the installed local version so the phone can recommend the current tunnel even if remote JS fetching is slow. The desktop pairing page shows local/stable/tunnel app versions before pairing. | Complete |
...
| Public Netlify UI and API | Production Netlify deploy `6a3770f49529294af075ee70` is ready. Static `relay-chat.js` currently serves version `2026.06.21.8`, and `/api/relay/status` returns `ok: true`. Deploy attempts `6a37729c8ad80d76f3966a44` and `6a37804334af399f98db2849` were skipped by Netlify because account credit usage was exceeded, so the local frontend version `2026.06.21.12`, expanded cleanup-name matcher, app-info route advice, and latest source docs are installed locally but not published to Netlify yet. The old `mobile-relay` function name is now compatibility-wrapped to the current implementation. | API usable; latest public UI blocked by Netlify credits |
...
tunnel URL: https://004d37c6e2e5a6.lhr.life/relay-chat.html
fallback tunnel URL: localtunnel, only when its status is fresh and healthy
app-info: local=2026.06.21.12, stable=2026.06.21.8, temporary=2026.06.21.12, recommended_source=temporary
...
```sh
cd "/Users/vincentpan/Library/Application Support/CodexRelayCloud"
node scripts/cloud-relay-localtunnel-install.cjs e2e '诊断'
...
- Current public tunnel UI: `https://b1d406918c5243.lhr.life/relay-chat.html`
